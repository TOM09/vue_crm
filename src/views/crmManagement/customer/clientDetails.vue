...
<template>
  <div class="clientDetails">
    ...
    <el-dialog title="编辑跟进人" :visible.sync="dialogFormVisible1" :before-close="handleClose" :modal="false">
      <el-form :model="updPartner">
        <el-form-item  label="跟进人"  class="s_client_item s_client_sel_divMargin0" style="margin-left: 80px;">
          <el-select v-model="updPartner.partner" multiple clearable filterable placeholder="请选择" class="s_order_search s_order_search2 el-form-item__content">
            <el-option
                v-for="item in persons"
                :key="item.dd_id"
                :label="item.nickname"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //编辑跟进人
      CoinCharge() {
         this.$post('client/updPartner',this.updPartner)
          .then((data) => {
            if(data.code) {
              this.$message({
                message: '修改成功！',
      ...
      clientAdd () {
        this.$get( 'client/detail',{id:this.$route.params.id})
          .then( (data) => {
            ...
            this.ruleForm.client_id.push(client.id);
            this.updPartner.client_id = client.id;
            ...
            this.person_choice = client.person_choice
            ...
            this.updPartner.partner = client.partner_edit ? client.partner_edit : [];
